...
const { results, resultsScore } = storeToRefs(metrics)

let resultStyle = computed(() => {
  return results.value.label === "Graduate" ? "color: green;" : "color: red"
})
...
async function goToAdvice(route: string | null){
  ...
  const optimumCategory = results.value.optimumCategory as keyof typeof routeNames;
...
<template>
  <v-dialog v-model="displayResults" width="500">
    <v-card>
      ...
      <h2 :style="resultStyle">{{ resultsScore }}% </h2>
